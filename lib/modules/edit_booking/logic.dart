...
class EditTableLogic extends GetxController {
    ...
    if (file == null) {
      try {
        FirebaseFirestore.instance.collection('tables').doc(id).update({
          ...
          'table_chairs_count':
              int.parse(tableChairsCountController.text.toString()),
          'image': downloadURL,
        });
  ...
  setData(DocumentSnapshot? productModel) {
    ...
    tableChairsCountController = TextEditingController(
        text: productModel.get('table_chairs_count').toString());
    tableReserveFromController = TextEditingController(
        text: productModel
            .get('reservation_from')
            .toDate()
            .toString()
            .substring(0, 16));
    tableReserveToController = TextEditingController(
        text: productModel
            .get('reservation_to')
            .toDate()
            .toString()
